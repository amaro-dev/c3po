        ...
        try {
            ...
            if (allDevices != null) {
                for (device in allDevices) {
                    LOG.info("Device details: serial=${device.serialNumber}, state=${device.state}, name=${device.name}, isOnline=${device.isOnline}")
                }
            }

            // Filter for online devices only
            val onlineDevices = allDevices?.filter { device ->
