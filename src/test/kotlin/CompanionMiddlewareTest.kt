...
class CompanionMiddlewareTest {
    @Test
    fun `Handle SelectDevice action when there's a new device`() = runBlocking {
    ...
    @Test
    fun `Handle SelectDevice action when it's the same device`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true)
        val middleware = CompanionMiddleware(commander)
        val device: AdbDevice = mockk(relaxed = true)
        val state = AppState(currentDevice = device)
    ...
    @Test
    fun `Handle Install action when has a current device`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true) {
    ...
    @Test
    fun `Handle Install action when install fails`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true) {
    ...
    @Test
    fun `Handle Install action when there's no current device`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true)
        val settings = Properties().apply { put(Settings.ADB_PATH_PROP, "") }
        val middleware = CompanionMiddleware(commander)
    ...
    @Test
    fun `Handle Prepare action when has a current device`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true) {
    ...
    @Test
    fun `Handle Prepare action when procedure fails`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true) {
    ...
    @Test
    fun `Handle Prepare action when there's no current device`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true)
        val settings = Properties().apply { put(Settings.ADB_PATH_PROP, "") }
        val middleware = CompanionMiddleware(commander)
        val state = AppState(settings = settings)
    ...
    @Test
    fun `Handle CheckInstalled action when companion app is installed`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true) {
        ...
        verify {
            ...
            processor.reduce(Action.Companion.UpdateState(CompanionState().setIsInstalled().setCheckedForPresence()))
        }
    }

    @Test
    fun `Handle CheckInstalled action when companion app is NOT installed`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true) {
    ...
    @Test
    fun `Handle CheckInstalled action when there's no current device`() = runBlocking {
        val commander: CompanionCommander = mockk(relaxed = true)
        val settings = Properties().apply { put(Settings.ADB_PATH_PROP, "") }
        val middleware = CompanionMiddleware(commander)
        val state = AppState(settings = settings)
