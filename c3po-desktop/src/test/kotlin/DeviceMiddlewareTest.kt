...
class DeviceMiddlewareTest {
    @Test
    fun `Handle RefreshDevice command when single device encountered`() =
        runTest {
            val device: AdbDevice = mockk(relaxed = true)
            val commander: CommandExecutor =
                mockk(relaxed = true) {
    ...
    @Test
    fun `Handle RefreshDevice command when multiple devices encountered`() =
        runTest {
            val device: AdbDevice = mockk(relaxed = true)
            val commander: CommandExecutor =
                mockk(relaxed = true) {
    ...
    @Test
    fun `Handle RefreshDevice command when fails to list devices`() =
        runTest {
            val commander: CommandExecutor =
                mockk(relaxed = true) {
    ...
    @Test
    fun `Handle CommandAction triggers running state`() =
        runTest {
    ...
    @Test
    fun `Handle Non-CommandAction does not trigger running state`() =
        runTest {
            val commander: CommandExecutor = mockk(relaxed = true)
            val middleware = DeviceMiddleware(commander)
            val state = AppState()
...
class AnyOfTypeMatcher<T>(
    private val expectedType: KClass<*>,
) : Matcher<T> {
    override fun match(arg: T?): Boolean = arg?.let { it.javaClass == expectedType.java } ?: false
}
